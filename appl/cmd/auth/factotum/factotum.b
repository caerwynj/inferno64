...
reads(str: string, off: int, nbytes: int): (array of byte, string)
{
	...
	if(off < 0 || off >= slen)
		return (nil, nil);
	if(off + nbytes > slen)
		nbytes = slen - off; # TODO potential bug truncating off to int from big
	if(nbytes <= 0)
		return (nil, nil);
	return (bstr[off:off+nbytes], nil); # TODO potential bug truncating off to int from big
}
