...
server(fio: ref Sys->FileIO, data: array of byte)
{
	...
	(offset, count, nil, rc) := <-fio.read =>
		if (rc != nil) {
			if (offset > len data)
				...
				rc <-= (data[offset:end], nil); # TODO potential bug truncating big to int
			}
		}
	(offset, d, nil, wc) := <-fio.write =>
		if (wc != nil) {
			...
			if (end > len data) {
				ndata := array[end] of byte; # TODO potential bug truncating big to int
				ndata[0:] = data;
				data = ndata;
				ndata = nil;
			}
			data[offset:] = d; # TODO potential bug truncating big to int
			wc <-= (len d, nil);
		}
	}
}
