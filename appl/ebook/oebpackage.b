...
open(f: string, warnings: chan of (Xml->Locator, string)): (ref Package, string)
{
	...
	* =>
	...
	# scan prolog for DOCTYPE and/or <package> tag.
	# EPUB 2 OPF files may omit the DOCTYPE entirely.
	pkgtag: ref Xml->Item.Tag;
	for(i := 0; i < 5; i++){
		xi = x.next();
		if(xi == nil)
			return (nil, "not a valid package file");
		pick d := xi {
		Text =>
			;	# whitespace between prolog items
		Doctype =>
			if(d.public && len d.params >= 2){
				case doctype(hd tl d.params, Laxchecking) {
				OEBpkg =>
					;	# OEB package, continue to find <package>
				OEBdoc =>
					...
					return (p, nil);
				* =>
					;	# unknown DOCTYPE, try to continue as EPUB 2
				}
			}
		Tag =>
			if(d.name == "package")
	...
	tag: ref Xml->Item.Tag;
	if(pkgtag != nil)
		tag = pkgtag;
	else if((tag = next(x, "package")) == nil)
		return (nil, "can't find package element");
	p.uniqueid = tag.attrs.get("unique-identifier");
	p.version = tag.attrs.get("version");
	...
	while((tag = next(x, nil)) != nil){
		...
		"metadata" =>
			while((tag = next(x, nil)) != nil){
				if(tag.name == "dc-metadata"){
					# OEB 1.0.1: metadata inside dc-metadata wrapper
					x.down();
					while((tag = next(x, nil)) != nil && (s := text(x)) != nil)
						p.meta = (tag.name, tag.attrs, s) :: p.meta;
					x.up();
				} else if(len tag.name > 3 && tag.name[:3] == "dc:"){
					# EPUB 2: dc:title etc. directly under metadata
					if((s := text(x)) != nil)
						p.meta = (tag.name, tag.attrs, s) :: p.meta;
				}
			}
		...
		"spine" =>
			p.toc = tag.attrs.get("toc");	# EPUB 2: NCX item id
...
Package.locate(p: self ref Package): int
{
	...
	# resolve NCX toc file if specified
	if(p.toc != nil){
		ncx := lookitem(p.manifest, p.toc);
		if(ncx != nil && ncx.file != nil)
			p.ncxfile = ncx.file;
	}
	return nmissing;
}
