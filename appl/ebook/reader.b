...
OEBpkg, OEBdoc, XHTMLdoc: con iota;
Laxchecking: con 1;
...
startdocument(d: ref Datasource): string
{
	...
	if (err != nil)
		error(d, err);
	dt := doctype(dtype, Laxchecking);
	if (dt != OEBdoc && dt != XHTMLdoc && dtype != "")
	...
	for (xi := x.next(); xi != nil; xi = x.next()) {
		...
		Doctype =>
			if (i.public && len i.params >= 2)
				dtd = hd tl i.params;
			# else: minimal DOCTYPE (e.g. <!DOCTYPE html>), dtd stays ""
		Stylesheet =>
# XXX			etc etc.
		Error =>
...
e_style(d: ref Datasource, i: ref Item.Tag)
{
	...
	# accept nil type (HTML5/XHTML default is CSS)
	if (ltype != nil && ltype != "text/x-oeb1-css" && ltype != "text/css") {
...
doctype(s: string, lax: int): int
{
	case s {
	"" =>
		return XHTMLdoc;	# no DOCTYPE: assume XHTML (EPUB 2)
	OEBpkgtype =>
		return OEBpkg;
	OEBdoctype =>
		return OEBdoc;
