...
usage()
{
	sys->fprint(stderr, "usage: ebook [-m] [-x] bookfile[.opf|.epub]\n");
	raise "fail:usage";
}
...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	if (extract && len bf > 5 && bf[len bf - 5:] == ".epub") {
		home := readenv("home");
		if (home == nil) home = "/usr/" + readenv("user");
		bn := bf;
		# get basename
		for (i := len bn - 1; i >= 0; i--)
			if (bn[i] == '/') { bn = bn[i+1:]; break; }
		bn = bn[0:len bn - 5]; # remove .epub
		tdir := home + "/ebooks/" + bn;
		(ok, nil) := sys->stat(tdir + "/META-INF/container.xml");
		if (ok < 0) {
			sys->print("Extracting %s to %s...\n", bf, tdir);
			warningch = chan of (Xml->Locator, string);
			spawn warningproc(warningch);
			(epf, err) := epubcontainer->open(bf, warningch);
			if (err == nil) {
				copydir("/mnt/epub", tdir);
				epubcontainer->close();
			} else {
				sys->fprint(stderr, "extract: %s\n", err);
			}
		}
		bf = tdir;
	}
	...
	(win, ctlchan) := tkclient->toplevel(ctxt, nil, bf, Tkclient->Hide);
	cch := chan of string;
...
#	fittoscreen(win);
	...
	(book, e) := Book.new(bf, win, ".d", "evch", size, indexprogress);
	if (book == nil) {
		pageprogress <-= nil;
	...
	# EPUB 2: build TOC menu from NCX if available
	if (book.pkg.ncxfile != nil) {
		(navpoints, nerr) := epubnav->parse(book.pkg.ncxfile, warningch);
		if (nerr != nil)
			sys->fprint(stderr, "ebook: NCX: %s\n", nerr);
		else if (navpoints != nil) {
			# resolve NCX hrefs relative to the NCX file's directory
			ncxdir := "";
			for (ni := len book.pkg.ncxfile; --ni >= 0;)
				if (book.pkg.ncxfile[ni] == '/') {
					ncxdir = book.pkg.ncxfile[:ni+1];
					break;
				}
			tocguide: list of ref OEBpackage->Reference;
			for (; navpoints != nil; navpoints = tl navpoints) {
				np := hd navpoints;
				nhref := np.href;
				if (nhref != nil && len nhref > 0 && nhref[0] != '/')
					nhref = ncxdir + nhref;
				tocguide = ref OEBpackage->Reference("toc", np.label, nhref) :: tocguide;
			}
			# reverse to restore order
			rev: list of ref OEBpackage->Reference;
			for (; tocguide != nil; tocguide = tl tocguide)
				rev = hd tocguide :: rev;
			makemenu(win, ".f.toc", "TOC", rev);
			cmd(win, "pack .f.toc -before .f.pagenum -side left");
		}
	}
...
loadmods()
{
	...
	epubcontainer = load Epubcontainer Epubcontainer->PATH;
	if (epubcontainer == nil)
		badmodule(Epubcontainer->PATH);
	epubcontainer->init(xml);

	epubnav = load Epubnav Epubnav->PATH;
	if (epubnav == nil)
		badmodule(Epubnav->PATH);
	epubnav->init(xml);
...
{
	opfpath := f;
	# if it's an .epub file, mount it and find the OPF inside
	if(len f > 5 && f[len f - 5:] == ".epub"){
		(p, err) := epubcontainer->open(f, warningch);
		if(err != nil)
			return (nil, "epub: " + err);
		opfpath = p;
	} else {
		...
		if (ok >= 0 && (d.mode & Sys->DMDIR)) {
			(p, err) := epubcontainer->parsecontainer(f + "/META-INF/container.xml", warningch);
			if (err != nil) return (nil, "container: " + err);
			opfpath = f + "/" + p;
		}
	}
	(pkg, e) := oebpackage->open(opfpath, warningch);
	if (pkg == nil)
		return (nil, e);
	nmissing := pkg.locate();
	if (nmissing > 0)
...
readenv(e: string): string
{
	if ((fd := sys->open("/env/" + e, Sys->OREAD)) != nil) {
		buf := array[128] of byte;
		n := sys->read(fd, buf, len buf);
		if (n > 0) return string buf[0:n];
	}
	return nil;
}
...
copydir(src, dst: string)
{
	...
	if (dir.mode & Sys->DMDIR) {
		sys->create(dst, Sys->OREAD, Sys->DMDIR | 8r755);
		...
		if(fd != nil){
			for(;;) {
				...
				if(n <= 0) break;
				for(i := 0; i < n; i++)
					copydir(src + "/" + d[i].name, dst + "/" + d[i].name);
			}
		}
	} else {
		sfd := sys->open(src, Sys->OREAD);
		if(sfd != nil){
			dfd := sys->create(dst, Sys->OWRITE, dir.mode);
			if(dfd != nil){
				buf := array[8192] of byte;
				while((n := sys->read(sfd, buf, len buf)) > 0)
					sys->write(dfd, buf, n);
			}
		}
	}
