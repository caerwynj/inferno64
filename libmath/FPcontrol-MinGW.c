...
getFPstatus(void)
{
	...
	if(fsr32&_EM_INEXACT) fsr |= INEX;
	if(fsr32&_EM_OVERFLOW) fsr |= OVFL;
	if(fsr32&_EM_UNDERFLOW) fsr |= UNFL;
	if(fsr32&_EM_ZERODIVIDE) fsr |= ZDIV;
	if(fsr32&_EM_INVALID) fsr |= INVAL;
	return fsr;
}
