 ...
 *  Adapted from 9front's devtls.c for Inferno.
 */
#include	"dat.h"
#include	"fns.h"
#include	"error.h"
...
struct OneWay
{
	...
	QLock		seclock;	/* locks secret parameters */
	u64int		seq;
...
struct TlsRec
{
	...
	/*
	 * function to generate authenticated data blob for different
	 * protocol versions
	 */
...
/* secure allocation helpers - zero memory before free */
static void*
secalloc(ulong n)
{
	return mallocz(n, 1);
...
tlswstat(Chan *c, uchar *dp, int n)
{
	...
	if(tr == nil)
		error(Ebadusefd);
	if(strcmp(tr->user, up->env->user) != 0)
		error(Eperm);
...
tlsbread(Chan *c, long n, ulong offset)
{
	...
	if(ty == Qdata){
		...
		if(tr->hprocessed == nil){
			...
			if(*b->rp++ == RAlert){
				kstrcpy(up->env->errstr, (char*)b->rp, ERRMAX);
				freeb(b);
				nexterror();
			}
			tr->hprocessed = b;
...
static Hashalg hashtab[] =
{
	...
	{ "sha256",	SHA256dlen,	initsha2_256key, },
	{ 0 }
};
...
static DigestState*
nomac(uchar *p, ulong len, uchar *key, ulong klen, uchar *digest, DigestState *s)
{
	USED(p); USED(len); USED(key); USED(klen); USED(digest);
	return s;
}
