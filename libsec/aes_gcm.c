...
/* rijndaelEncrypt is defined in aes.c (not static) */
extern void rijndaelEncrypt(const u32int rk[], int Nr, const uchar pt[16], uchar ct[16]);
...
static ulong
aesxctr1(AESGCMstate *s, uchar ctr[AESbsize], uchar *dat, ulong len)
{
	uchar tmp[AESbsize];
	ulong i;

	rijndaelEncrypt(s->aes.ekey, s->aes.rounds, ctr, tmp);
	if(len > AESbsize)
		len = AESbsize;
	for(i=0; i<len; i++)
		dat[i] ^= tmp[i];
...
static void
aesxctrn(AESGCMstate *s, uchar *dat, ulong len)
{
	uchar ctr[AESbsize];
	ulong i;

	memmove(ctr, s->aes.ivec, AESbsize);
	while(len > 0){
...
{
	if(ivlen == 96/8){
		memmove(s->aes.ivec, iv, ivlen);
		memset(s->aes.ivec+ivlen, 0, AESbsize-ivlen);
		s->aes.ivec[AESbsize-1] = 1;
	} else {
		ulong L[4], Y[4] = {0};
...
{
	setupAESstate(&s->aes, key, keylen, nil);

	memset(s->aes.ivec, 0, AESbsize);
	rijndaelEncrypt(s->aes.ekey, s->aes.rounds, s->aes.ivec, s->aes.ivec);
	load128(s->aes.ivec, s->gcmH);
	memset(s->aes.ivec, 0, AESbsize);
	prepareM(s->gcmH, s->gcmM);
